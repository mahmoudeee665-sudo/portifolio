...
const Header = () => {
  ...
  const handleSendWhatsApp = (ev) => {
    ...
    const url = `https://wa.me/${WHATSAPP_OWNER}?text=${encodeURIComponent(
      text
    )}`;
...
const handleNavClick = (e, id) => {
  ...
  // Close the mobile sheet first so layout is final
  setIsOpen(false);

  // Wait one frame so height collapse animation finishes, then scroll
          ...
          <div className="relative h-16 md:h-18 lg:h-20 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            ...
            >
              <div className="relative overflow-hidden rounded-full backdrop-blur-xl ring-1 ring-white/40 dark:ring-white/10 border border-white/40 dark:border-white/5 bg-gradient-to-r from-white/25 via-white/15 to-white/20 dark:from-white/10 dark:via-white/5 dark:to-white/10 shadow-[0_8px_30px_rgba(80,35,180,0.15)] px-4 md:px-5 py-2">
                ...
                <motion.span
                  ...
                  style={{
                    maskImage:
                      "radial-gradient(40% 50% at 50% 50%, #000 55%, transparent)",
                  }}
                />
                {/* soft blobs */}
                <motion.span
            ...
            <div className="absolute inset-x-0 flex justify-center md:hidden pointer-events-none">
              <motion.button
                ...
                className="group pointer-events-auto" // ✅ allow clicks only on the logo
              >
            ...
            >
              ...
              <form className="space-y-4" onSubmit={handleSendWhatsApp}>
                ...
                <div>
                  ...
                  />
                  {errors.msg && (
                    <p className="mt-1 text-xs text-red-500">{errors.msg}</p>
                  )}
                </div>
